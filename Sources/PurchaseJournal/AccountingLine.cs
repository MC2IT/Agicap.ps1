...
public class AccountingLine {
	...
	public static explicit operator AccountingLine(PSObject psObject) {
		...
		return new AccountingLine() {
			...
			ConvertedCreditAmount = Convert.AsDecimal(json.convertedCreditAmount),
			ConvertedDebitAmount = Convert.AsDecimal(json.convertedDebitAmount),
			Credit = Convert.AsDecimal(json.credit),
			Currency = json.currency as string ?? "EUR",
			Debit = Convert.AsDecimal(json.debit),
