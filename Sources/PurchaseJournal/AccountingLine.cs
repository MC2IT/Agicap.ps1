...
public class AccountingLine {
	...
	public static explicit operator AccountingLine(PSObject psObject) {
		...
		return new AccountingLine() {
			...
			AccountType = Enum.Parse<AccountingLineAccountType>(json.accountType as string ?? nameof(AccountingLineAccountType.ExpenseAccount)),
			AdditionalAnalyticalCodes = json.additionalAnalyticalCodes is PSObject additionalAnalyticalCodes ? additionalAnalyticalCodes.ToDictionary<string>() : new Dictionary<string, string>(),
			AnalyticalCodes = json.analyticalCodes is PSObject analyticalCodes ? analyticalCodes.ToDictionary<string>() : new Dictionary<string, string>(),
			ConversionRate = Number.AsDouble(json.conversionRate) ?? 0,
			ConvertedCreditAmount = Number.AsDecimal(json.convertedCreditAmount) ?? 0,
			ConvertedDebitAmount = Number.AsDecimal(json.convertedDebitAmount) ?? 0,
			Credit = Number.AsDecimal(json.credit) ?? 0,
			Currency = json.currency as string ?? "EUR",
			Debit = Number.AsDecimal(json.debit) ?? 0,
